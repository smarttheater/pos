    ...
    orders.forEach((order) => {
        order.acceptedOffers.forEach((acceptedOffer) => {
            const customData = {
                ...
                price: order.price,
                priceComponent:
                    (<any>acceptedOffer.priceSpecification).priceComponent ===
                    undefined
                        ? ''
                        : (<any>acceptedOffer.priceSpecification).priceComponent
                              .map((p: any) => {
                                  const name =
                                      typeof p.name === 'string'
                                          ...
                                          ? ''
                                          : p.name.ja;
                                  const referenceQuantityValue = (<any>p)
                                      .referenceQuantity?.value
                                      ? (<any>p).referenceQuantity.value
                                      : 1;
        ...
        acceptedOffers: {
            itemOffered: {
                ...
                reservationFor: {
                    ...
                    superEvent: {
                        location:
                            input.itemOfferedType === 'EventReservation'
                                ? {
                                      branchCodes:
                                          theater === undefined
                                              ? []
