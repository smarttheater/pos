...
export class AdmissionService {
    ...
    public selectScheduleDate(scheduleDate: string) {
        this.store.dispatch(
            admissionAction.selectScheduleDate({ scheduleDate })
        );
    }
    ...
    public async checkQrcode(params: {
        ...
        try {
            ...
            const qrcodeToken = await this.checkToken({
                code,
                screeningEvent,
                scheduleDate,
                entranceGate,
    ...
    public async checkToken(params: {
        ...
        if (searchResult.data.length > 1 || searchResult.data.length === 0) {
                ...
                location: {
                    identifier:
                        entranceGate === undefined
                            ? undefined
                            : typeof entranceGate.identifier !== 'string'
                            ? undefined
    ...
    public async getScreeningEvent(
        ...
        return new Promise<void>((resolve, reject) => {
            this.store.dispatch(
                admissionAction.getScreeningEvent({ screeningEvent })
            );
